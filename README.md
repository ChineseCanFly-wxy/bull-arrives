...
任意股票一键出分：**6 因子加权 0–100 分**，附各项因子明细与指标快照（MA/MACD/RSI/KDJ/BOLL/ATR 等），结论可追溯到具体数据。

**不止给分，还给能照着做的价位**：

- **操作计划** —— 建仓区间、止损价、止盈价、盈亏比、按单笔风险反推的仓位上限，每条价位都写明是怎么算出来的；当前不满足入场条件时会直接说「还差什么」
- **三套交易规则** —— 趋势跟随 / 均值回归 / 放量突破，各自的性格写在界面上：
...
- **策略自带规则** —— 每条筛选策略配套一条交易规则（超跌反弹配均值回归、强势突破配放量突破…），
  筛选器负责粗筛，打开个股分析时自动用对应规则算买点
