...
- **5 套内置预设** —— 全部 / 稳健趋势 / 强势突破 / 短线活跃 / 超跌反弹，一键切换
- **自定义区间** —— 价格、市值、换手率、涨跌幅、量比、成交额自由组合
- **选股范围与预设解耦** —— 板块勾选（沪/深/创/科/北）与排除规则（ST/退市/停牌/一字板）**独立记忆**，切换预设不会被重置
- **条件持久化** —— 上次的预设、板块、排除、数据源选择全部记住，重启即恢复
- **服务端分页** —— 先显示命中数量，点「展示数据」再展开；翻页逐页取数、稳定排序不重复；每页 20/50/100 可调（设置里也能配）
- **数据源可切换** —— 自动（默认，多通道回退）/ 东方财富 / 新浪财经
...
- **全市场筛选器** —— 5 套预设 + 自定义区间，服务端分页，条件全持久化
- **推荐榜** —— 活跃股多因子评分排行
- **个股量化评分** —— 6 因子 0–100 分 + 指标快照
- **智能监控** —— ATR 自动止损止盈位盯盘
