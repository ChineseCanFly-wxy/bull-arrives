...
## 📌 v1.3.0 更新要点：不只是看盘，开始帮你选股

> 本次更新新增 **全市场筛选器 / 推荐榜 / 个股量化评分 / 智能监控** 四大模块，
> 并把数据层重写为**多通道自动回退**——单一数据源被限流或阻断时自动切换，不再整页报错。
...
- [x] 全市场筛选器（5 套预设 + 自定义区间 + 板块/排除独立记忆 + 服务端分页）
- [x] 推荐榜（活跃股多因子评分排行）
- [x] 个股量化评分（6 因子 0–100 分 + 指标快照）
- [x] 智能监控（ATR 自动止损止盈位）
- [x] 数据层多通道回退（东财/新浪快照，腾讯/新浪/东财日K）
- [x] 一字板判定按板块区分涨跌幅限制，补上 ST 一字板
- [x] 设置页显示版本 / 构建时间 / 程序位置，前端日志可落盘
...
> 下面第一条点进去是**完整方案文档**（含各数据源的实测结论、分阶段路线图与风险清单）。
