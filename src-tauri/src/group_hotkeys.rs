use crate::{datasource::DataSourceManager, db::Database};
...
fn build_status(db: &Database, keys: &[Option<Shortcut>; 2]) -> GroupHotkeyStatus {
    let entry = |index: usize| GroupHotkeyEntry {
        hotkey: db
            .get_setting(SETTING_KEYS[index])
            .ok()
            .flatten()
            .unwrap_or_default(),
...
#[tauri::command]
pub fn get_group_hotkey_status(
    app: tauri::AppHandle,
...
pub fn handle(app: &tauri::AppHandle, shortcut: &Shortcut) -> bool {
    ...
    let index = state
        ...
        .iter()
        .position(|s| s.as_ref() == Some(shortcut));
    ...
    match db.get_group_snapshot() {
        Ok(snapshot) => {
            ...
            let current = ids
                .iter()
                .position(|id| *id == snapshot.active_group_id)
                .unwrap_or(0);
            ...
            if let Err(error) = db.select_watch_group(ids[target]) {
                log::warn!("快捷键切组失败: {}", error);
            } else {
...
#[tauri::command]
pub fn set_group_hotkey(
    app: tauri::AppHandle,
    db: State<'_, Arc<Database>>,
    direction: String,
    hotkey: String,
) -> Result<GroupHotkeyStatus, String> {
    ...
    let candidate = if trimmed.is_empty() {
        ...
        Some(
            trimmed
                .parse::<Shortcut>()
                .map_err(|_| "快捷键格式无法识别，请重新录制".to_string())?,
        )
    };
...
pub fn restore(app: &tauri::AppHandle, db: &Database) {
    ...
    for (index, key) in SETTING_KEYS.iter().enumerate() {
        let Some(raw) = db.get_setting(key).ok().flatten() else {
            continue;
        };
