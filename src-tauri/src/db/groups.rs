...
impl Database {
    ...
    pub fn get_current_watch_codes(&self) -> SqliteResult<Vec<(String, String)>> {
        Ok(self
            ...
            .into_iter()
            .map(|item| (item.code, item.market))
            .collect())
    }
    ...
    pub fn get_current_watchlist(&self) -> SqliteResult<Vec<WatchItem>> {
        ...
        let valid = id > 0
            && conn.query_row(
                "SELECT EXISTS(SELECT 1 FROM watch_groups WHERE id=?1)",
                [id],
                |r| r.get::<_, bool>(0),
            )?;
        let sql = if valid {
    ...
    pub fn get_group_snapshot(&self) -> SqliteResult<GroupSnapshot> {
        ...
        let active_group_id = if groups.iter().any(|g| g.id == saved) {
            saved
        } else {
            0
        };
    ...
    pub fn move_current_group_item(&self, watch_id: i64, direction: &str) -> Result<(), String> {
        ...
        let index = ids
            .iter()
            .position(|id| *id == watch_id)
            .ok_or("股票不在当前分组")?;
        let target = match direction {
            "top" => 0,
            "up" => index.saturating_sub(1),
            _ => (index + 1).min(ids.len() - 1),
        };
        ids.remove(index);
        ...
        for (order, id) in ids.iter().enumerate() {
            tx.execute(
                "UPDATE watch_group_members SET sort_order=?1 WHERE group_id=?2 AND watch_id=?3",
                params![order as i64, snapshot.active_group_id, id],
            )
            .map_err(|e| e.to_string())?;
        }
    ...
    pub fn delete_watch_group(&self, id: i64) -> Result<(), String> {
        if id <= 0 {
            return Err("全部分组不能删除".into());
        }
        let conn = self.conn.lock().unwrap_or_else(|e| e.into_inner());
        let tx = conn.unchecked_transaction().map_err(|e| e.to_string())?;
        tx.execute("DELETE FROM watch_group_members WHERE group_id=?1", [id])
            .map_err(|e| e.to_string())?;
        tx.execute("DELETE FROM watch_groups WHERE id=?1", [id])
            .map_err(|e| e.to_string())?;
        tx.execute(
            "UPDATE settings SET value='0' WHERE key='active_group_id' AND value=?1",
            [id.to_string()],
        )
        .map_err(|e| e.to_string())?;
        tx.commit().map_err(|e| e.to_string())
    ...
    pub fn set_group_member(
        ...
        if group_id <= 0 {
            return Err("请操作自定义分组；全部自动包含所有自选".into());
        }
        let conn = self.conn.lock().unwrap_or_else(|e| e.into_inner());
