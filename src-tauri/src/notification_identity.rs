...
mod platform {
    ...
    unsafe fn initialize_com() -> Result<ComGuard, String> {
        CoInitializeEx(None, COINIT_APARTMENTTHREADED)
            .ok()
            .map_err(|e| format!("初始化 Windows Shell COM 失败: {e}"))?;
        Ok(ComGuard)
    }
    ...
    unsafe fn load_link(path: &Path) -> Result<(IShellLinkW, IPropertyStore), String> {
        ...
        let persist: IPersistFile = link
            .cast()
            .map_err(|e| format!("读取快捷方式接口失败: {e}"))?;
        persist
            .Load(
                PCWSTR(wide(path.as_os_str()).as_ptr()),
                windows::Win32::System::Com::STGM(0),
            )
            .map_err(|e| format!("读取现有快捷方式失败: {e}"))?;
        let store: IPropertyStore = link
            .cast()
            .map_err(|e| format!("读取快捷方式属性失败: {e}"))?;
        Ok((link, store))
    }
    ...
    pub fn register() -> Result<NotificationIdentityStatus, String> {
        ...
        unsafe {
            ...
            if path.exists() && !existing_owned(&path, &exe)? {
                return Err(format!(
                    "开始菜单中已存在非本程序拥有的同名快捷方式，未覆盖: {}",
                    path.display()
                ));
            }
            if let Some(parent) = path.parent() {
                fs::create_dir_all(parent).map_err(|e| format!("创建开始菜单目录失败: {e}"))?;
            }
            let link: IShellLinkW = CoCreateInstance(&ShellLink, None, CLSCTX_INPROC_SERVER)
                .map_err(|e| format!("创建 ShellLink COM 对象失败: {e}"))?;
            ...
            let store: IPropertyStore = link
                .cast()
                .map_err(|e| format!("打开快捷方式属性失败: {e}"))?;
            let value = PROPVARIANT::from(APP_USER_MODEL_ID);
            store
                .SetValue(&PKEY_APP_USER_MODEL_ID, &value)
                .map_err(|e| format!("写入 AUMID 失败: {e}"))?;
            store
                .Commit()
                .map_err(|e| format!("保存 AUMID 失败: {e}"))?;
            let persist: IPersistFile = link
                .cast()
                .map_err(|e| format!("打开快捷方式保存接口失败: {e}"))?;
            persist
...
mod platform {
    ...
    pub fn status() -> Result<NotificationIdentityStatus, String> {
        Ok(NotificationIdentityStatus {
            ...
            registered: false,
            shortcut_path: None,
            detail: "仅 Windows 支持通知身份注册".into(),
        })
    }
    pub fn register() -> Result<NotificationIdentityStatus, String> {
        Err("仅 Windows 支持通知身份注册".into())
    }
}

#[tauri::command]
pub fn get_notification_identity_status() -> Result<NotificationIdentityStatus, String> {
    platform::status()
}

/// This mutating command is never called at startup; registration occurs only after an explicit UI click.
#[tauri::command]
pub fn register_notification_identity() -> Result<NotificationIdentityStatus, String> {
    platform::register()
}
